...
class Router {
    ...
    static function __callStatic($name, $args) {
        if(strtolower($_SERVER["REQUEST_METHID"]) == $name) {
            self::$pages[] = $args;
        }
    }
    ...
    static function start() {
        ...
        foreach(self::$pages as $page) {
            ...
            if(preg_match("/^{$regex}$/", $currentURL, $matchs)) {
                unset($matchs[0]);

                if($permission) {
                    if($permission == "gest" && user()) go("/", "로그인한 회원은 접근할 수 없습니다.");
                    if($permission == "user" && !user()) go("/", "로그인 후 이용하실 수 있습니다.");
